...
class Validator {
	 ...
	 * only "rules" and "messages" are supported. Specifying these into an array will easily
	 ...
	 *
	 * The jquery validator plugin allows keys to point to functions. This is not supported in the
	 ...
	 * obviously cannot. To integrate client and server as described, make sure all keys
	 ...
	 * @return bool is the element valid
	...
	public function optional($sField) {
		...
		if (
			...
			isset($this->asClassRule[$sField]['required']) // this could be a conditional requirement
		) {
			$bRequired = false === $this->oMethodCollection->requiredCondinally(
				$this->asValue[$sField],
				$this->asClassRule[$sField]['required'],
				$this->asValue
			);
		}
		return $bRequired;
	 ...
	 * Format is gerenated according to options provided to the constructor
	 *
	...
	public function validate(array $asValue) {
		...
		foreach ($this->asClassRule as $sField => $asRules) {
			if (false === isset($asValue[$sField]) &&	# this field was not validated, as it was not present in the list
...
class ValidatorMethodCollection extends ArrayObject {
	...
	# regular expressions denoting various validation patterns
  ...
  (
	 ...
	 * the first paramater in Validator::addMethod ($sName). The second parameter is the
	...
	/**
	 * From the jquery validation documentation:
	...
	public function required($sValue, $mParam = null, $asFullList = array()) {
		if (false === is_null($mParam)) {
			// if it is NOT required conditionally, it is valid
			// false means: there is a condition, but it is not met, so it is not required
			// true means: there is a condition, it is met, so this is required
			if (false === $this->requiredCondinally($sValue, $mParam, $asFullList)) {
				return true;
			}
	...
	/**
	 * Should only be called when it is know that this element is required
	 * It could be that the element is conditionally required. This method
	 * will check whether:
	 * 1. such a condition is set at all
	 * 2. the condition applies
	 *
	 * @return bool true if it is required, i.e. there is no condition OR the
	 * condition is met
	 */
	public function requiredCondinally($sValue, $mParam = null, $asFullList = array()) {
		$bRequired = true;
		if (false === is_null($mParam)) {
			...
			if (is_callable($mParam)) {	# custom required function
				...
				# iff true, it IS required, otherwise: not
				$bRequired = $mParam($sValue, $asFullList);
			}
			...
			elseif (is_string($mParam)) {
				...
				# Condition NOT met iff
				if (
					false === isset($asFullList[$oJQuery->sFieldname]) ||	# 	the field is not there (i.e. not checked)
					true === empty($asFullList[$oJQuery->sFieldname])		# 	the field is empty (i.e. select/input)
				) {
					return false;											# not required given the conditions!
				}
			}
		}
		return true;														# required in all other cases
	}
